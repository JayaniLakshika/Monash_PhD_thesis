...
res_bw  <- simulateResiduals(glmm_model)
res_min <- simulateResiduals(glmm_model_min)

df_bw <- tibble::tibble(
  fitted = res_bw$fittedPredictedResponse,
  resid  = res_bw$scaledResiduals
)

df_min <- tibble::tibble(
  fitted = res_min$fittedPredictedResponse,
  resid  = res_min$scaledResiduals
)

fitvsres_bw <- ggplot(df_bw, aes(x = fitted, y = resid)) +
  geom_point(alpha = 0.4) +
  geom_smooth(se = FALSE) +
  labs(x = "Fitted values", y = "Scaled residuals") +
  theme_minimal()

resid_bw <- ggplot(df_bw, aes(x = resid)) +
  geom_histogram(bins = 30) +
  labs(x = "Scaled residuals", y = "Count") +
  theme_minimal()

resid_qq_bw <- ggplot(df_bw, aes(sample = resid)) +
  stat_qq(distribution = qunif) +
  stat_qq_line(distribution = qunif) +
  theme_minimal()

re_bw <- ranef(glmm_model)$subject[[1]]

subj_bw <- ggplot(data.frame(re = re_bw), aes(sample = re)) +
  stat_qq() +
  stat_qq_line() +
  labs(y = "Random effects (subject)") +
  theme_minimal()
```

```{r}
fitvsres_bw + resid_bw + 
  resid_qq_bw + subj_bw +
  plot_layout(ncol = 2)
```
