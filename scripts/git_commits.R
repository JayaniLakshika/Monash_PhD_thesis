...
my_pkgs <- c("quollr", "cardinalR")

quollr <- get_commits("~/Rpkg/quollr") %>%
  mutate(Package = my_pkgs[1])
cardinalR <- get_commits("~/Rpkg/cardinalR") %>%
  mutate(Package = my_pkgs[2])

pkgs <- bind_rows(quollr, cardinalR) %>%
