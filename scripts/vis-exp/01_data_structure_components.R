#### This script contains all the data structure components
...
# # Function to gen a filled corn-shaped cluster in 4D space with a circular base and an offset
# gen_filled_corn_cluster <- function(n, height = 5, base_radius = 3, tip_point = c(0, 0, 0, 0)) {
#   if (n <= 0) {
#     stop("Number of points should be a positive number.")
#   }
#
...
#
#   # Radius decreases linearly from the base to the tip
#   radii <- base_radius * (1 - (height_values / height))
#
#   # gen points uniformly distributed in the circular base at each height level
#   theta <- runif(n, 0, 2 * pi)
#   r <- sqrt(runif(n)) * radii  # Radius within the circle (sqrt to ensure uniform distribution)
#
#   x <- r * cos(theta)
#   y <- r * sin(theta)
#   z <- height_values
#
#   # Combine dimensions into a 4D structure
#   filled_corn_cluster <- cbind(x, y, z, height_values)
#
#   # Shift the cluster towards the tip point
#   filled_corn_cluster <- sweep(filled_corn_cluster, 2, tip_point, "+")
#
#   return(filled_corn_cluster)
# }
