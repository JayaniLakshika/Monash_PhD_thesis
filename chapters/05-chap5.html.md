...
Nonlinear dimension reduction (NLDR) methods such as tSNE, UMAP, PHATE, TriMAP, and PaCMAP are commonly used to visualize high-dimensional data, yet their effectiveness for human perception of structure remains unclear. In particular, $2\text{-}D$ NLDR layouts can be difficult to interpret when clusters differ in shape, number of points, and separation. This study evaluates how well NLDR methods preserve perceptually meaningful cluster structure using a controlled simulation and a human recognition experiment. We generated three-cluster data with distinct geometries and unequal sizes using the `cardinalR` package, varying inter-cluster separation. Participants decided whether a $2\text{-}D$ NLDR layout and a tour represented the same high-dimensional data. Cluster separability was quantified using both global and local measures, including the between-to-within (BW) ratio and the exponential scaled minimum inter-cluster distance. The results show clear differences across methods. UMAP and PaCMAP benefit from increased separation, leading to higher identification accuracy, while TriMAP shows a weaker trend. In contrast, tSNE and PHATE often distort global structure, resulting in persistent misidentification. These findings highlight the need to assess NLDR methods based on how effectively their visualizations support human perception of high-dimensional structure.
...
This study provides empirical evidence that NLDR methods differ substantially in how well they preserve high-dimensional structures that are perceptually meaningful for clustering. By combining a controlled simulation of three clusters with varying separation, shape, and size, and a human recognition experiment, we quantified how structural separability in the high-dimension space translates into correct identification of clusters in $2\text{-}D$ representations.
...
Analyses across data structures and participants indicate that misidentification is not driven by random error or a small subset of individuals. Instead, it reflects systematic interactions between data geometry and method-specific distortions. Certain curved or dense structures remain difficult to recognize even under large separation, particularly when NLDR layouts obscure global shape cues or introduce artificial fragmentation. Visual comparisons of tSNE and UMAP layouts reinforce this finding: while UMAP generally preserves geometric continuity more effectively, neither method guarantees clear perceptual separation for all structures.

Overall, these findings highlight that increased cluster separation alone does not ensure improved interpretability of NLDR visualizations. Method-specific behavior and the geometric properties of the data jointly determine perceptual outcomes. This suggests that evaluations of NLDR methods should account not only for algorithmic performance but also for how visual representations support human interpretation, particularly in settings where users rely on visual cues to assess structure in high-dimensional data.
