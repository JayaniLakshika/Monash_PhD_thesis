...
#| fig-alt: "A design shows the visual layout of an experiment for ten subjects, arranged in ten rows (one per subject) and 15 columns (one per trial). Each object represents a single trial. Along the horizontal axis, trials are ordered from 1 to 20; the vertical axis lists subjects 1 through 5. Most cells (15 per subject) are coded as SAME trials, where the same data structure is shown in both the 2‑D NLDR plot and the tour. SAME trials are further distinguished by combinations of five NLDR methods (tSNE, UMAP, PHATE, PaCMAP, TriMAP) and five distance scale factors (small to large), with 15 balanced method–scale combinations per subject. The grid visually emphasizes that each subject has the same number of trials and a consistent pattern of many SAME trials."
...
plot_data <- experiment_design01 |>
  ...
  mutate(subject = factor(subject, levels = sort(unique(subject), decreasing = TRUE)),
...
# Plot
ggplot(plot_data, aes(x = trial_pos, y = subject)) +
  geom_point(aes(
...
min_dist_quasi <- ggplot(data = results_df_method_ds,
  ...
  ylab("exp(scaled minimum distance)") +
  xlab("distance scale factor") +
  theme_minimal() +
  theme(aspect.ratio = 0.4)
```
...
nldr1_three_clust_07_tsne <- get_embeddings(dt_structutre = "three_clust_07") |>
  filter(scale_factor == c(0.1, 1.1)) |>
  filter(method %in% c("tsne")) |>
...
prop_df_07_tsne <- results_df_method_ds |>
  ...
  mutate(
    label = sprintf("%.2f", prop_correct)
  )

corner_df_07_tsne <- nldr1_three_clust_07_tsne |>
...
annot_df_07_tsne <- corner_df_07_tsne |>
  left_join(
    prop_df_07_tsne,
    by = c("method" = "method",
...
sf_max_xy_07_tsne <- nldr1_three_clust_07_tsne |>
...
annot_df_07_tsne <- annot_df_07_tsne |>
  select(-x, -y) |>          # remove old per-method values
  left_join(sf_max_xy_07_tsne, by = "scale_factor")


nld_plt1_three_clust_07_tsne <- ggplot(
  data = nldr1_three_clust_07_tsne,
  aes(
    x = emb1,
    y = emb2
  )
...
nldr1_three_clust_07_umap <- get_embeddings(dt_structutre = "three_clust_07") |>
  filter(scale_factor == c(0.1, 1.1)) |>
  filter(method %in% c("umap")) |>
  mutate(scale_factor = recode(scale_factor,
                               `0.1` = "S",
                               `1.1` = "L")) |>
  mutate(scale_factor = factor(scale_factor,
...
prop_df_07_umap <- results_df_method_ds |>
  filter(structure_high_d == "three_clust_07") |>
  group_by(method, distance_factor) |>
  summarise(
    n_total   = n(),
...
corner_df_07_umap <- nldr1_three_clust_07_umap |>
  group_by(method, scale_factor) |>
  summarise(
    x = max(emb1, na.rm = TRUE),
    y = max(emb2, na.rm = TRUE),
...
annot_df_07_umap <- corner_df_07_umap |>
  left_join(
    prop_df_07_umap,
    by = c("method" = "method",
           "scale_factor" = "distance_factor")
...
sf_max_xy_07_umap <- nldr1_three_clust_07_umap |>
  group_by(scale_factor) |>
  summarise(
    x = max(emb1, na.rm = TRUE),
    y = max(emb2, na.rm = TRUE),
...
annot_df_07_umap <- annot_df_07_umap |>
  select(-x, -y) |>          # remove old per-method values
  left_join(sf_max_xy_07_umap, by = "scale_factor")


nld_plt1_three_clust_07_umap <- ggplot(
  data = nldr1_three_clust_07_umap,
  aes(
    x = emb1,
    y = emb2
  )
...
(three_clust07_proj1/ three_clust07_proj1_1) | (three_clust07_proj2/ three_clust07_proj2_1) | nld_plt1_three_clust_07_tsne | nld_plt1_three_clust_07_umap
```
...
nldr1_three_clust_13_tsne <- get_embeddings(dt_structutre = "three_clust_13") |>
  filter(scale_factor == c(0.1, 1.1)) |>
  filter(method %in% c("tsne")) |>
  mutate(scale_factor = recode(scale_factor,
                               `0.1` = "S",
...
prop_df_13_tsne <- results_df_method_ds |>
  filter(structure_high_d == "three_clust_13") |>
  group_by(method, distance_factor) |>
  summarise(
    n_total   = n(),
    n_correct = sum(result == 1, na.rm = TRUE),
...
corner_df_13_tsne <- nldr1_three_clust_13_tsne |>
  group_by(method, scale_factor) |>
  summarise(
    x = max(emb1, na.rm = TRUE),
    y = max(emb2, na.rm = TRUE),
...
annot_df_13_tsne <- corner_df_13_tsne |>
  left_join(
    prop_df_13_tsne,
    by = c("method" = "method",
           "scale_factor" = "distance_factor")
...
sf_max_xy_13_tsne <- nldr1_three_clust_13_tsne |>
  group_by(scale_factor) |>
  summarise(
    x = max(emb1, na.rm = TRUE),
    y = max(emb2, na.rm = TRUE),
...
annot_df_13_tsne <- annot_df_13_tsne |>
  select(-x, -y) |>          # remove old per-method values
  left_join(sf_max_xy_13_tsne, by = "scale_factor")


nld_plt1_three_clust_13_tsne <- ggplot(
  data = nldr1_three_clust_13_tsne,
  aes(
    x = emb1,
    y = emb2
  )
...
nldr1_three_clust_13_umap <- get_embeddings(dt_structutre = "three_clust_13") |>
  filter(scale_factor == c(0.1, 1.1)) |>
  filter(method %in% c("umap")) |>
  mutate(scale_factor = recode(scale_factor,
                               `0.1` = "S",
...
prop_df_13_umap <- results_df_method_ds |>
  filter(structure_high_d == "three_clust_13") |>
  group_by(method, distance_factor) |>
  summarise(
    n_total   = n(),
...
corner_df_13_umap <- nldr1_three_clust_13_umap |>
  group_by(method, scale_factor) |>
  summarise(
    x = max(emb1, na.rm = TRUE),
    y = max(emb2, na.rm = TRUE),
...
annot_df_13_umap <- corner_df_13_umap |>
  left_join(
    prop_df_13_umap,
    by = c("method" = "method",
           "scale_factor" = "distance_factor")
...
sf_max_xy_13_umap <- nldr1_three_clust_13_umap |>
  group_by(scale_factor) |>
  summarise(
    x = max(emb1, na.rm = TRUE),
    y = max(emb2, na.rm = TRUE),
...
annot_df_13_umap <- annot_df_13_umap |>
  select(-x, -y) |>          # remove old per-method values
  left_join(sf_max_xy_13_umap, by = "scale_factor")


nld_plt1_three_clust_13_umap <- ggplot(
  data = nldr1_three_clust_13_umap,
  aes(
    x = emb1,
    y = emb2
  )
...
centered_three_clust_13_data <- center_data(three_clust_13_data) |>
  as_tibble()
...
projected_df <- get_projection(projection = prj1, 
                               centered_data = centered_three_clust_13_data, 
...
three_clust13_proj1_1 <- plot_proj(
  proj_obj = projected_df, 
  point_param = c(1, 0.05, "#000000"), # size, alpha, color
  plot_limits = c(-0.4, 0.55), 
  title = "b1", 
  cex = 1, 
  axis_text_size = 2,
  is_color = FALSE) + 
...
three_clust13_proj2_1 <- plot_proj(
  proj_obj = projected_df, 
  point_param = c(0.5, 0.05, "#000000"), # size, alpha, color
  plot_limits = c(-0.55, 0.5), 
  title = "b2", 
  cex = 1, 
  axis_text_size = 2,
  is_color = FALSE) + 
