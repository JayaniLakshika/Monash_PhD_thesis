...
Looking more closely, the data contain six clusters that are fairly close to one another (`r knitr::asis_output(if (knitr::is_html_output()) { "@fig-dt-tourr-pbmc"} else { "@fig-dt-prj-pbmc"})`). Three of them have clearly nonlinear shapes, two Gaussian blobs, and one is closer to an ellipse, with a bit of background noise scattered around. The full dataset contains $\approx160,000$ observations; however, for visualization, we used a random subset of $10,000$ points to reduce computational overhead and keep both interactive and static displays responsive, while still preserving the overall cluster structure. These kinds of data structures are common in bioinformatics data. 
...
We generated a synthetic $10\text{-}D$ dataset using the cardinalR package to mimic the structure of the PBMC CITE-seq data (`r knitr::asis_output(if (knitr::is_html_output()) { "@fig-dt-tourr-sim"} else { "@fig-dt-prj-sim"})`). The dataset consists of six clusters with the following number of points: $500$, $700$, $700$, $500$, $700$, and $700$, for a total of $3,800$ points. The cluster centers were arranged according to a custom distance matrix to control their relative positions in the space. Each cluster was assigned a different geometric shape: three approximately Gaussian clusters, one quadratic cluster, one spherical spiral, and one conic spiral, with additional background noise.
...
To match the dimensionality of the PBMC dataset, six additional noise dimensions were added with small variance:
...
This produced a $10\text{-}D$ dataset with a mix of nonlinear and Gaussian structures, similar to the PBMC CITE-seq data, while keeping a bit of background noise for realism.
