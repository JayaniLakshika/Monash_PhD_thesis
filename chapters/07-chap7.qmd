...
Using the `cardinalR` package, we generated a synthetic dataset designed to mimic the structure of the PBMC CITE-seq data (`r knitr::asis_output(if (knitr::is_html_output()) { "@fig-dt-tourr-sim"} else { "@fig-dt-prj-sim"})`). We first created six clusters with different shapes—three approximately Gaussian, one quadratic, one spherical spiral, and one conic spiral—while adding background noise:
