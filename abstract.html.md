...
This research presents five original contributions. The first contribution, developed by (@sec-first-paper), introduces a new method for visualizing how NLDR warps data. This method will improve the diagnostics of NLDR techniques. The second contribution, detailed in [@sec-second-paper], involves implementing [@sec-first-paper]'s method as an R package. The third contribution, described in [@sec-third-paper], provides evidence in identification of clusters at various distances when observing NLDR representation and the tour view of high-dimensional data. This finding is based on a human subject experiment that explores both the perception and misperception of NLDR representations. The fourth contribution, outlined in [@sec-fourth-paper], introduces an R package designed to generate high-dimensional data structures. This package improves characteristics such as adding noise dimensions and background noise. Finally, the fifth contribution, presented in [@sec-fifth-paper], features a Shiny app that offers a user-friendly interface for analysts to obtain the most accurate NLDR representation. Overall, this work advances the field of diagnosing NLDR by enhancing the visualization of high-dimensional data.
